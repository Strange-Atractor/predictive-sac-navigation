...
# ==========================================================
# FUNCIÓN PRINCIPAL: ENTRENAMIENTO SAC PREDICTIVO
# ==========================================================
def main():
    ...
    if EJECUTAR_VERIFICACION_PREVIA_SAC_PREDICTIVO:

        verificar_entrenamiento_sac_predictivo()
    ...
    DIRECTORIO_ENTRENAMIENTO_SAC_PREDICTIVO.mkdir(
        parents=True,
        exist_ok=True,
    )

    DIRECTORIO_GRAFICAS_ENTRENAMIENTO_SAC_PREDICTIVO.mkdir(
        parents=True,
        exist_ok=True,
    )

    # ------------------------------------------------------
    # 3. Configurar semillas
    # ------------------------------------------------------
    ...
    # ------------------------------------------------------
    # 4. Cargar únicamente el actor reactivo mejorado
    # ------------------------------------------------------

    ruta_actor_inicial = (
        RUTA_ACTOR_INICIAL_ENTRENAMIENTO_SAC_PREDICTIVO
    )

    if not ruta_actor_inicial.is_file():

        raise FileNotFoundError(
            "No se encontró el actor reactivo mejorado que "
            "inicializa el SAC predictivo:\n"
            f"{ruta_actor_inicial}"
        )
    ...
    if not carga_actor_correcta:
        ...
        raise RuntimeError(
            "El actor reactivo inicial no se cargó "
            "correctamente."
        )

    # ------------------------------------------------------
    # 5. Críticos, objetivos y temperatura nuevos
    # ------------------------------------------------------
    ...
    print(
        "TTC en la recompensa: True"
    )

    print(
        "Actor reactivo modificado: False"
    )

    print(
        "Críticos, buffer y temperatura reutilizados: False"
    )
    ...
    print(
        "Semillas:",
        f"{SEMILLA_BASE_ENTRENAMIENTO_SAC_PREDICTIVO} a "
        f"{SEMILLA_BASE_ENTRENAMIENTO_SAC_PREDICTIVO + NUMERO_EPISODIOS_ENTRENAMIENTO_SAC_PREDICTIVO - 1}",
    )

    print(
        "Transiciones aleatorias iniciales:",
        TRANSICIONES_ALEATORIAS_INICIALES_SAC_PREDICTIVO,
    )
    ...
    print(
        "Rango:",
        f"{SEMILLA_BASE_VALIDACION_SAC_PREDICTIVO} a "
        f"{SEMILLA_BASE_VALIDACION_SAC_PREDICTIVO + NUMERO_SEMILLAS_VALIDACION_SAC_PREDICTIVO - 1}",
    )

    print(
        "Las semillas 70000–70099 continúan reservadas."
    )

    # ------------------------------------------------------
    # 9. Entrenamiento predictivo
    # ------------------------------------------------------
    ...
    resultado_entrenamiento = (
        entrenar_agente_sac_predictivo_con_validacion_periodica(
            numero_episodios=(
                NUMERO_EPISODIOS_ENTRENAMIENTO_SAC_PREDICTIVO
            ),
            episodios_entre_validaciones=(
            ...
            actualizaciones_por_paso=(
                ACTUALIZACIONES_POR_PASO_SAC_PREDICTIVO
            ),
            factor_descuento=FACTOR_DESCUENTO_SAC,
            tau=TAU_POLYAK_SAC,
            entropia_objetivo=ENTROPIA_OBJETIVO_SAC,
            ventana_promedio=(
                VENTANA_PROMEDIO_SAC_PREDICTIVO
            ),
            dt=DT,
            dispositivo=DISPOSITIVO_SAC,
        )
    ...
    informacion_metadatos_final = (
        agregar_metadatos_checkpoint_final_sac_predictivo(
            ruta_checkpoint=(
                RUTA_CHECKPOINT_FINAL_SAC_PREDICTIVO
            ),
            resultado_entrenamiento=resultado_entrenamiento,
    ...
    # ------------------------------------------------------
    # 12. Diagnóstico independiente del mejor actor
    # ------------------------------------------------------
    ...
    resultado_diagnostico = evaluar_episodio_sac_predictivo(
        semilla=SEMILLA_DIAGNOSTICA_SAC_PREDICTIVO,
        actor=actor_mejor,
        pasos_maximos=PASOS_MAXIMOS_SEGUIMIENTO,
        dt=DT,
        dispositivo=DISPOSITIVO_SAC,
    ...
    checkpoint_mejor_correcto = (
        RUTA_MEJOR_ACTOR_SAC_PREDICTIVO.is_file()
        and checkpoint_mejor.get(
            "tipo"
        )
        == "mejor_actor_sac_predictivo"
        and checkpoint_mejor.get(
            "variante_sac"
        )
        == "predictivo"
    ...
    print(
        "RESULTADO DEL ENTRENAMIENTO SAC PREDICTIVO"
    )
    ...
    print(
        "Semilla:",
        SEMILLA_DIAGNOSTICA_SAC_PREDICTIVO,
    )
    ...
    print(
        "Actor reactivo inicial cargado:",
        carga_actor_correcta,
    )

    print(
        "Checkpoint predictivo correcto:",
        checkpoint_mejor_correcto,
    )
    ...
    if todo_correcto:
        ...
        print(
            "\nRESULTADO TÉCNICO: ENTRENAMIENTO PREDICTIVO COMPLETADO"
        )
    ...
    if MOSTRAR_RESULTADOS_ENTRENAMIENTO_SAC_PREDICTIVO:
