...
# ==========================================================
# PRUEBA INDEPENDIENTE DEL SAC REACTIVO MEJORADO
# ==========================================================

NUMERO_SEMILLAS_PRUEBA_REACTIVO_MEJORADO = 100
SEMILLA_BASE_PRUEBA_REACTIVO_MEJORADO = 65000
FRECUENCIA_IMPRESION_PRUEBA_REACTIVO_MEJORADO = 10

RUTA_MEJOR_ACTOR_PRUEBA_REACTIVO_MEJORADO = Path(
    "resultados_sac/"
    "tercera_corrida_reactivo_mejorado/"
    "checkpoint_mejor_actor_reactivo_mejorado.pt"
)

DIRECTORIO_PRUEBA_REACTIVO_MEJORADO = Path(
    "resultados_sac/"
    "prueba_independiente_reactivo_mejorado"
)

RUTA_CSV_PRUEBA_REACTIVO_MEJORADO = (
    DIRECTORIO_PRUEBA_REACTIVO_MEJORADO
    / "resultados_100_semillas.csv"
)

RUTA_CSV_FALLOS_PRUEBA_REACTIVO_MEJORADO = (
    DIRECTORIO_PRUEBA_REACTIVO_MEJORADO
    / "semillas_fallidas.csv"
)

RUTA_RESUMEN_PRUEBA_REACTIVO_MEJORADO = (
    DIRECTORIO_PRUEBA_REACTIVO_MEJORADO
    / "resumen_prueba_independiente.txt"
)

RUTA_GRAFICA_PRUEBA_REACTIVO_MEJORADO = (
    DIRECTORIO_PRUEBA_REACTIVO_MEJORADO
    / "distribucion_resultados_100_semillas.png"
)

# Estos umbrales son los mismos utilizados para decidir si
# un SAC está suficientemente robusto antes de compararlo.
TASA_EXITO_MINIMA_REACTIVO_MEJORADO = 0.85
TASA_COLISION_DINAMICA_MAXIMA_REACTIVO_MEJORADO = 0.10
TASA_COLISION_ESTATICA_MAXIMA_REACTIVO_MEJORADO = 0.10
TASA_FUERA_MAPA_MAXIMA_REACTIVO_MEJORADO = 0.05
TASA_TIMEOUT_MAXIMA_REACTIVO_MEJORADO = 0.05


def guardar_resumen_prueba_reactivo_mejorado(
    resumen,
    checkpoint,
    criterios,
    semillas_fallidas,
    ...
    lineas = [
        ...
        "=" * 72,
        "",
        f"Checkpoint: {RUTA_MEJOR_ACTOR_PRUEBA_REACTIVO_MEJORADO}",
        f"Episodio del actor: {checkpoint.get('episodio', 'desconocido')}",
        ...
        "Entrenamiento durante esta prueba: no",
        "Predicción: no",
        "TTC: no",
        "",
        "RESULTADOS",
        ...
        f"Colisiones estáticas: {conteos.get('colision_estatica', 0)} "
        f"({100.0 * resumen['tasa_colision_estatica']:.2f}%)",
        f"Colisiones dinámicas: {conteos.get('colision_dinamica', 0)} "
        f"({100.0 * resumen['tasa_colision_dinamica']:.2f}%)",
        f"Fuera del mapa: {conteos.get('fuera_mapa', 0)} "
        f"({100.0 * resumen['tasa_fuera_mapa']:.2f}%)",
        f"Timeout: {conteos.get('timeout', 0)} "
        f"({100.0 * resumen['tasa_timeout']:.2f}%)",
        "",
        "MÉTRICAS GENERALES",
        "-" * 72,
        f"Recompensa media: {resumen['recompensa_media']:.6f}",
        f"Pasos promedio: {resumen['pasos_promedio']:.3f}",
        f"Distancia final media: {resumen['distancia_final_media']:.6f} m",
        f"Error medio de ruta promedio: "
        f"{resumen['error_medio_ruta_promedio']:.6f} m",
        f"Clearance mínimo promedio: "
        f"{resumen['clearance_minimo_promedio']:.6f} m",
        f"Clearance mínimo absoluto: "
        f"{resumen['clearance_minimo_absoluto']:.6f} m",
        "",
        "MÉTRICAS DE EPISODIOS EXITOSOS",
        "-" * 72,
        f"Tiempo promedio: {resumen['tiempo_promedio_exitos']:.6f} s",
        f"Longitud promedio: {resumen['longitud_promedio_exitos']:.6f} m",
        f"Error medio de ruta: "
        f"{resumen['error_ruta_promedio_exitos']:.6f} m",
        f"Clearance mínimo promedio: "
        f"{resumen['clearance_minimo_promedio_exitos']:.6f} m",
        "",
        ...
        "",
        f"Semillas fallidas ({len(semillas_fallidas)}):",
        str(
            semillas_fallidas
        ),
    ]
    ...
    if not ruta_salida.is_file():
        ...
        raise RuntimeError(
            "No fue posible guardar el resumen de la prueba."
        )
...
def graficar_distribucion_prueba_reactivo_mejorado(
    resumen,
    ruta_guardado,
    dpi=300,
):
    ...
    eje.set_title(
        "SAC reactivo mejorado: resultados en 100 semillas nuevas"
    )
    ...
    if not ruta_guardado.is_file():

        raise RuntimeError(
            "No fue posible guardar la gráfica de resultados."
        )
...
def main():
    ...
    # ------------------------------------------------------
    # 1. Comprobar el checkpoint
    # ------------------------------------------------------

    if not RUTA_MEJOR_ACTOR_PRUEBA_REACTIVO_MEJORADO.is_file():

        raise FileNotFoundError(
            "No se encontró el mejor actor reactivo mejorado:\n"
            f"{RUTA_MEJOR_ACTOR_PRUEBA_REACTIVO_MEJORADO}"
        )

    checkpoint = torch.load(
        RUTA_MEJOR_ACTOR_PRUEBA_REACTIVO_MEJORADO,
        map_location="cpu",
        weights_only=False,
    )

    # ------------------------------------------------------
    # 2. Validar que se cargó el modelo correcto
    # ------------------------------------------------------
    ...
    if not checkpoint_correcto:
        ...
        raise ValueError(
            "El checkpoint no corresponde al SAC reactivo "
            "mejorado esperado."
        )
    ...
    DIRECTORIO_PRUEBA_REACTIVO_MEJORADO.mkdir(
        parents=True,
        exist_ok=True,
    )
    ...
    print(
        "Número:",
        NUMERO_SEMILLAS_PRUEBA_REACTIVO_MEJORADO,
    )

    print(
        "Rango:",
        SEMILLA_BASE_PRUEBA_REACTIVO_MEJORADO,
        "a",
        SEMILLA_BASE_PRUEBA_REACTIVO_MEJORADO
        + NUMERO_SEMILLAS_PRUEBA_REACTIVO_MEJORADO
        - 1,
    ...
    print(
        "Las semillas finales 70000–70099 permanecen reservadas."
    )

    # ------------------------------------------------------
    # 4. Evaluar 100 episodios sin entrenamiento
    # ------------------------------------------------------
    ...
    resultado_prueba = evaluar_actor_sac_multisemilla(
        actor=actor,
        semilla_base=(
            SEMILLA_BASE_PRUEBA_REACTIVO_MEJORADO
        ),
        ...
        dispositivo=DISPOSITIVO_SAC,
        frecuencia_impresion=(
            FRECUENCIA_IMPRESION_PRUEBA_REACTIVO_MEJORADO
        ),
    )

    resumen = resultado_prueba[
        "resumen"
    ]

    # ------------------------------------------------------
    # 5. Aplicar los criterios de robustez
    # ------------------------------------------------------
    ...
    criterios = {
        "exito": bool(
            ...
            >= TASA_EXITO_MINIMA_REACTIVO_MEJORADO
        ),
        "colision_dinamica": bool(
            resumen[
                "tasa_colision_dinamica"
            ]
            <= TASA_COLISION_DINAMICA_MAXIMA_REACTIVO_MEJORADO
        ),
        "colision_estatica": bool(
            resumen[
                "tasa_colision_estatica"
            ]
            <= TASA_COLISION_ESTATICA_MAXIMA_REACTIVO_MEJORADO
        ),
        "fuera_mapa": bool(
            resumen[
                "tasa_fuera_mapa"
            ]
            <= TASA_FUERA_MAPA_MAXIMA_REACTIVO_MEJORADO
        ),
        "timeout": bool(
            resumen[
                "tasa_timeout"
            ]
            <= TASA_TIMEOUT_MAXIMA_REACTIVO_MEJORADO
        ),
    }
    ...
    ruta_csv = guardar_resultados_validacion_sac_csv(
        resultado_validacion=resultado_prueba,
        ruta_csv=RUTA_CSV_PRUEBA_REACTIVO_MEJORADO,
    )

    informacion_fallos = guardar_fallos_prueba_independiente_sac(
        resultado_validacion=resultado_prueba,
        ruta_csv=RUTA_CSV_FALLOS_PRUEBA_REACTIVO_MEJORADO,
    )

    ruta_resumen = guardar_resumen_prueba_reactivo_mejorado(
        resumen=resumen,
        checkpoint=checkpoint,
        criterios=criterios,
        semillas_fallidas=informacion_fallos[
            "semillas_fallidas"
        ],
        ruta_salida=RUTA_RESUMEN_PRUEBA_REACTIVO_MEJORADO,
    )

    informacion_grafica = graficar_distribucion_prueba_reactivo_mejorado(
        resumen=resumen,
        ruta_guardado=RUTA_GRAFICA_PRUEBA_REACTIVO_MEJORADO,
        dpi=300,
    )
    ...
    sin_entrenamiento = (
        actor_sin_cambios
        and actor_sin_gradientes
    )

    todo_tecnicamente_correcto = (
        checkpoint_correcto
        and carga_estricta_correcta
        and parametros_finitos
        and numero_resultados_correcto
        and conteos_completos
        and semillas_correctas
        and archivos_correctos
        and sin_entrenamiento
    )
    ...
    print(
        "RESULTADO DE LA PRUEBA INDEPENDIENTE REACTIVA MEJORADA"
    )
    ...
    print(
        "Éxitos:",
        f"{resumen['numero_exitos']}/"
        f"{resumen['numero_semillas']}",
    )
    ...
    print(
        "Actor sin gradientes:",
        actor_sin_gradientes,
    )

    if todo_tecnicamente_correcto:
    ...
    if criterios[
        ...
        print(
            "RESULTADO CIENTÍFICO: SAC REACTIVO MEJORADO APTO"
        )

    else:

        print(
            "RESULTADO CIENTÍFICO: SAC REACTIVO MEJORADO AÚN NO APTO"
        )
