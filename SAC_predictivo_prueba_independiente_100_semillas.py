...
NUMERO_EPISODIOS_ENTRENAMIENTO_SAC_PREDICTIVO = 400

SEMILLA_BASE_ENTRENAMIENTO_SAC_PREDICTIVO = 60000

TRANSICIONES_ALEATORIAS_TERCERA_CORRIDA_SAC = 5000

ACTUALIZACIONES_POR_PASO_TERCERA_CORRIDA_SAC = 1

EPISODIOS_ENTRE_VALIDACIONES_SAC_PREDICTIVO = 20

NUMERO_SEMILLAS_VALIDACION_SAC_PREDICTIVO = 50

SEMILLA_BASE_VALIDACION_SAC_PREDICTIVO = 40000

VENTANA_PROMEDIO_TERCERA_CORRIDA_SAC = 25

TASA_APRENDIZAJE_ACTOR_SAC_PREDICTIVO = 1e-4

TASA_APRENDIZAJE_CRITICOS_SAC_PREDICTIVO = 3e-4

TASA_APRENDIZAJE_ALPHA_SAC_PREDICTIVO = 3e-4
...
def agregar_metadatos_checkpoint_final_reactivo_mejorado(
    ...
    checkpoint[
        "configuracion_entrenamiento_predictivo"
    ] = {
        "episodios": int(
...
# ==========================================================
# SAC PREDICTIVO: CONFIGURACIÓN SEPARADA
# ==========================================================
#
# Estas funciones NO sustituyen ni modifican las funciones
# del SAC reactivo mejorado. El entorno reactivo conserva:
#
#   construir_parche_egocentrico_sac
#   construir_observacion_sac
#   reiniciar_entorno_sac
#   ejecutar_paso_entorno_sac
#
# El SAC predictivo utiliza exclusivamente las funciones que
# terminan en "_predictivo".

HORIZONTES_PREDICCION_PARCHE_SAC = (
    0.0,
    0.5,
    1.0,
    1.5,
...
INTENSIDADES_PREDICCION_PARCHE_SAC = (
    1.00,
    0.75,
    0.50,
    0.25,
...
PASO_PREDICCION_DINAMICA_SAC = DT

HORIZONTE_RIESGO_DINAMICO_SAC = 2.0

CLEARANCE_DINAMICO_PREDICHO_OBJETIVO_SAC = 0.60

TTC_OBJETIVO_SAC = 1.50

VELOCIDAD_BAJA_RIESGO_PREDICTIVO_SAC = 0.08

PESO_SAC_CLEARANCE_DINAMICO_PREDICHO = 1.50

PESO_SAC_TTC_DINAMICO_PREDICHO = 3.00

PESO_SAC_AVANCE_RIESGOSO_PREDICTIVO = 2.00

PESO_SAC_DETENCION_TRAYECTORIA_PREDICTIVA = 2.50

RUTA_ACTOR_REACTIVO_INICIAL_SAC_PREDICTIVO = Path(
    "resultados_sac/"
    "tercera_corrida_reactivo_mejorado/"
    "checkpoint_mejor_actor_reactivo_mejorado.pt"
)

DIRECTORIO_SAC_PREDICTIVO = Path(
    "resultados_sac/sac_predictivo"
)


# ==========================================================
# PREDECIR OBSTÁCULOS DINÁMICOS
# ==========================================================
def predecir_obstaculos_dinamicos_sac_predictivo(
    obstaculos_dinamicos,
    obstaculos_estaticos,
    tiempo_actual,
    horizontes=HORIZONTES_PREDICCION_PARCHE_SAC,
    paso_prediccion=PASO_PREDICCION_DINAMICA_SAC,
):
    ...
    if (
        ...
        or tiempo_actual < 0.0
    ):

        raise ValueError(
            "El tiempo actual de predicción no es válido."
        )
    ...
    if (
        ...
        or paso_prediccion <= 0.0
    ):

        raise ValueError(
            "El paso de predicción debe ser positivo."
        )
    ...
    if len(
        ...
        raise ValueError(
            "Debe existir al menos un horizonte predictivo."
        )
    ...
    if any(
        ...
        raise ValueError(
            "Los horizontes predictivos deben ser finitos "
            "y no negativos."
        )
    ...
    if any(
        ...
        raise ValueError(
            "Los horizontes predictivos deben estar "
    ...
    for horizonte_objetivo in horizontes:
        ...
        predicciones.append(
            {
                ...
                "tiempo_absoluto": float(
                    tiempo_actual
                    + horizonte_objetivo
                ),
                "obstaculos": [
                    obstaculo.copy()
                    for obstaculo in estados_simulados
                ],
            }
        )
...
# ==========================================================
# PARCHE EGOCÉNTRICO DEL SAC PREDICTIVO
# ==========================================================
def construir_parche_egocentrico_sac_predictivo(
    estado_robot,
    obstaculos_estaticos,
    obstaculos_dinamicos,
    tiempo_actual=0.0,
    horizontes_prediccion=HORIZONTES_PREDICCION_PARCHE_SAC,
    intensidades_prediccion=INTENSIDADES_PREDICCION_PARCHE_SAC,
):

    horizontes_prediccion = tuple(
        float(
            valor
        )
        for valor in horizontes_prediccion
    )

    intensidades_prediccion = tuple(
        float(
            valor
        )
        for valor in intensidades_prediccion
    )

    if len(
        horizontes_prediccion
    ) != len(
        intensidades_prediccion
    ):

        raise ValueError(
            "Cada horizonte predictivo debe tener una "
            "intensidad asociada."
        )
    ...
    if any(
        intensidad < 0.0
        or intensidad > 1.0
        for intensidad in intensidades_prediccion
    ):

        raise ValueError(
            "Las intensidades predictivas deben estar "
            "dentro del intervalo [0, 1]."
        )

    # El parche reactivo se usa únicamente como base para
    # conservar exactamente el mismo canal estático y la
    # ocupación dinámica actual con intensidad 1.0.

    parche_reactivo = construir_parche_egocentrico_sac(
        estado_robot=estado_robot,
        obstaculos_estaticos=obstaculos_estaticos,
        obstaculos_dinamicos=obstaculos_dinamicos,
    )

    canal_estatico = parche_reactivo[
        0
    ].copy()
    ...
    predicciones = (
        predecir_obstaculos_dinamicos_sac_predictivo(
            obstaculos_dinamicos=obstaculos_dinamicos,
            obstaculos_estaticos=obstaculos_estaticos,
            tiempo_actual=tiempo_actual,
            horizontes=horizontes_prediccion,
            paso_prediccion=(
                PASO_PREDICCION_DINAMICA_SAC
            ),
        )
    )
    ...
    if parche.shape != forma_esperada:

        raise ValueError(
            "El parche predictivo tiene una forma "
            "incorrecta."
        )
    ...
    if not np.all(
        ...
        raise ValueError(
            "El parche predictivo contiene valores no "
            "finitos."
        )
...
# ==========================================================
# OBSERVACIÓN DEL SAC PREDICTIVO
# ==========================================================
def construir_observacion_sac_predictivo(
    estado_robot,
    submeta,
    meta,
    obstaculos_estaticos,
    ...
    parche = construir_parche_egocentrico_sac_predictivo(
        estado_robot=estado_robot,
        obstaculos_estaticos=obstaculos_estaticos,
        obstaculos_dinamicos=obstaculos_dinamicos,
        tiempo_actual=tiempo_actual,
    ...
    # Los seis escalares permanecen exactamente iguales a los
    # del SAC reactivo. Así la única diferencia de observación
    # está en el canal dinámico temporal.
...
# ==========================================================
# RIESGO DINÁMICO PREDICTIVO
# ==========================================================
def calcular_riesgo_dinamico_predictivo_sac(
    estado_robot,
    velocidad_lineal,
    velocidad_angular,
    obstaculos_dinamicos,
    obstaculos_estaticos,
    tiempo_actual,
    horizonte=HORIZONTE_RIESGO_DINAMICO_SAC,
    paso_prediccion=PASO_PREDICCION_DINAMICA_SAC,
):
    ...
    if (
        ...
        raise ValueError(
            "El horizonte y el paso predictivo deben ser "
            "positivos."
        )

    estado_simulado = tuple(
        float(
            valor
        )
        for valor in estado_robot
    )

    obstaculos_simulados = [
        obstaculo.copy()
        for obstaculo in obstaculos_dinamicos
    ]

    clearance_actual = calcular_clearance_dinamico(
        estado_simulado,
        obstaculos_simulados,
    )
    ...
    tiempo_clearance_minimo = 0.0

    estado_clearance_minimo = estado_simulado

    obstaculos_clearance_minimo = [
        obstaculo.copy()
        for obstaculo in obstaculos_simulados
    ]
    ...
    clearance_anterior = float(
        clearance_actual
    )

    while tiempo_simulado < horizonte - 1e-12:
        ...
        estado_simulado = actualizar_estado_robot(
        ...
        clearance_nuevo = float(
            calcular_clearance_dinamico(
                estado_simulado,
                obstaculos_simulados,
            )
        )
        ...
        if (
            ...
            velocidad_cierre_maxima = max(
                velocidad_cierre_maxima,
                velocidad_cierre,
            )

        if clearance_nuevo < clearance_minimo:

            clearance_minimo = clearance_nuevo

            tiempo_clearance_minimo = float(
                tiempo_simulado
            )

            estado_clearance_minimo = tuple(
                estado_simulado
            )
        ...
        if (
            ...
            and clearance_anterior > 0.0
            and clearance_nuevo <= 0.0
        ):
        ...
        clearance_anterior = clearance_nuevo

    colision_predicha = bool(
        np.isfinite(
            ttc_predicho
        )
        or clearance_minimo <= 0.0
    )

    return {
        "horizonte": horizonte,
        "paso_prediccion": paso_prediccion,
        "clearance_dinamico_actual": float(
            clearance_actual
        ),
        ...
        "estado_robot_clearance_minimo": (
            estado_clearance_minimo
        ),
        "obstaculos_clearance_minimo": (
            obstaculos_clearance_minimo
        ),
        "tiempos_prediccion": tiempos,
        "clearances_predichos": clearances,
        "colision_predicha": colision_predicha,
    }


# ==========================================================
# PENALIZACIÓN DEL RIESGO DINÁMICO PREDICTIVO
# ==========================================================
def calcular_penalizacion_riesgo_dinamico_predictivo_sac(
    riesgo_predictivo,
    velocidad_lineal,
):

    clearance_predicho = float(
        riesgo_predictivo[
            "clearance_dinamico_predicho_minimo"
        ]
    )
    ...
    if np.isfinite(
        ...
        riesgo_clearance = float(
            np.clip(
                (
                    CLEARANCE_DINAMICO_PREDICHO_OBJETIVO_SAC
                    - clearance_predicho
                )
                / max(
                    CLEARANCE_DINAMICO_PREDICHO_OBJETIVO_SAC,
    ...
    penalizacion_clearance = (
        PESO_SAC_CLEARANCE_DINAMICO_PREDICHO
        * riesgo_clearance
    )

    penalizacion_ttc = (
        PESO_SAC_TTC_DINAMICO_PREDICHO
        * riesgo_ttc
    )

    penalizacion_avance_riesgoso = (
        PESO_SAC_AVANCE_RIESGOSO_PREDICTIVO
        * riesgo_ttc
        * velocidad_normalizada ** 2
    )

    robot_detenido_o_muy_lento = (
        velocidad_lineal
        <= VELOCIDAD_BAJA_RIESGO_PREDICTIVO_SAC
    )

    trayectoria_de_colision = bool(
        riesgo_predictivo[
            "colision_predicha"
        ]
    )
    ...
    if (
        robot_detenido_o_muy_lento
        and trayectoria_de_colision
    ):

        penalizacion_detencion_trayectoria = (
            PESO_SAC_DETENCION_TRAYECTORIA_PREDICTIVA
            * max(
                riesgo_ttc,
                riesgo_clearance,
            )
        ...
        penalizacion_detencion_trayectoria = 0.0
    ...
    return {
        "riesgo_clearance_dinamico_predicho": (
            riesgo_clearance
        ),
        "riesgo_ttc_dinamico_predicho": (
            riesgo_ttc
        ),
        "velocidad_lineal_normalizada": (
            velocidad_normalizada
        ...
        "penalizacion_clearance_dinamico_predicho": (
            penalizacion_clearance
        ),
        "penalizacion_ttc_dinamico_predicho": (
            penalizacion_ttc
        ),
        "penalizacion_avance_riesgoso_predictivo": (
            penalizacion_avance_riesgoso
        ),
        "penalizacion_detencion_trayectoria_predictiva": (
            penalizacion_detencion_trayectoria
        ),
        "penalizacion_riesgo_dinamico_predictivo": float(
            penalizacion_total
        ),
    }
...
# ==========================================================
# REINICIAR EL ENTORNO DEL SAC PREDICTIVO
# ==========================================================
def reiniciar_entorno_sac_predictivo(
    semilla,
):

    # Se reutiliza la generación determinista del entorno
    # reactivo. Después se sustituye únicamente la observación
    # por su versión predictiva.
    ...
    observacion_predictiva = (
        construir_observacion_sac_predictivo(
            estado_robot=entorno[
                "estado_robot"
            ],
            submeta=entorno[
            ...
            obstaculos_dinamicos=entorno[
                "obstaculos_dinamicos"
            ],
            velocidad_lineal_actual=entorno[
                "velocidad_lineal_actual"
            ],
            velocidad_angular_actual=entorno[
                "velocidad_angular_actual"
            ],
            indice_progreso=entorno[
                "indice_progreso"
            ],
            numero_puntos_camino=len(
                entorno[
    ...
    riesgo_inicial = (
        calcular_riesgo_dinamico_predictivo_sac(
            estado_robot=entorno[
                "estado_robot"
            ],
            velocidad_lineal=0.0,
    ...
    entorno[
        "observacion_reactiva_inicial"
    ] = {
        "parche": observacion_reactiva[
            "parche"
        ].copy(),
        "escalares": observacion_reactiva[
            "escalares"
        ].copy(),
    }
    ...
    entorno[
        ...
        "parche": observacion_predictiva[
            "parche"
        ].copy(),
        "escalares": observacion_predictiva[
            "escalares"
        ].copy(),
    }

    entorno[
        "riesgo_dinamico_predictivo"
    ] = riesgo_inicial
...
# ==========================================================
# EJECUTAR UN PASO DEL ENTORNO SAC PREDICTIVO
# ==========================================================
def ejecutar_paso_entorno_sac_predictivo(
    entorno,
    accion,
    pasos_maximos=PASOS_MAXIMOS_SEGUIMIENTO,
    dt=DT,
    ...
    if entorno.get(
        ...
        raise ValueError(
            "El entorno entregado no fue reiniciado en modo "
            "predictivo."
        )

    # La transición física y la recompensa reactiva mejorada
    # permanecen idénticas. Esta llamada no añade predicción
    # al SAC reactivo; actúa sobre una instancia marcada como
    # predictiva y después sustituye su observación de salida.

    (
        observacion_reactiva_nueva,
        recompensa_reactiva,
        terminado,
        truncado,
        informacion,
    ...
    riesgo_predictivo = (
        calcular_riesgo_dinamico_predictivo_sac(
            ...
            velocidad_lineal=informacion[
                "velocidad_lineal"
            ],
            velocidad_angular=informacion[
                "velocidad_angular"
            ],
            obstaculos_dinamicos=entorno[
                "obstaculos_dinamicos"
    ...
    componentes_predictivos = (
        calcular_penalizacion_riesgo_dinamico_predictivo_sac(
    ...
    penalizacion_predictiva = float(
        componentes_predictivos[
            "penalizacion_riesgo_dinamico_predictivo"
        ]
    )

    recompensa_predictiva = float(
        recompensa_reactiva
        - penalizacion_predictiva
    )

    observacion_predictiva_nueva = (
        construir_observacion_sac_predictivo(
            estado_robot=entorno[
                "estado_robot"
            ],
    ...
    entorno[
        "observacion_reactiva_ultima"
    ] = {
        "parche": observacion_reactiva_nueva[
            "parche"
        ].copy(),
        "escalares": observacion_reactiva_nueva[
            "escalares"
        ].copy(),
    }
    ...
    entorno[
        ...
        "parche": observacion_predictiva_nueva[
            "parche"
        ].copy(),
        "escalares": observacion_predictiva_nueva[
            "escalares"
        ].copy(),
    }
    ...
    componentes_recompensa = informacion[
        "componentes_recompensa"
    ].copy()

    componentes_recompensa.update(
        riesgo_predictivo
    )
    ...
    componentes_recompensa[
        "recompensa_total"
    ] = recompensa_predictiva

    informacion.update(
        {
            "modo_sac": "predictivo",
            "recompensa_reactiva": float(
                recompensa_reactiva
            ),
            "recompensa_predictiva": (
                recompensa_predictiva
            ...
            "colision_dinamica_predicha": bool(
                riesgo_predictivo[
                    "colision_predicha"
                ]
            ),
            ...
            "recompensa": recompensa_predictiva,
        }
    )
...
# ==========================================================
# ENTRENAMIENTO DEL SAC PREDICTIVO
# ==========================================================

NUMERO_EPISODIOS_ENTRENAMIENTO_SAC_PREDICTIVO = 400

SEMILLA_BASE_ENTRENAMIENTO_SAC_PREDICTIVO = 80000

TRANSICIONES_ALEATORIAS_INICIALES_SAC_PREDICTIVO = 5000

ACTUALIZACIONES_POR_PASO_SAC_PREDICTIVO = 1

EPISODIOS_ENTRE_VALIDACIONES_SAC_PREDICTIVO = 20

NUMERO_SEMILLAS_VALIDACION_SAC_PREDICTIVO = 50

SEMILLA_BASE_VALIDACION_SAC_PREDICTIVO = 85000

VENTANA_PROMEDIO_SAC_PREDICTIVO = 25
...
RUTA_ACTOR_INICIAL_ENTRENAMIENTO_SAC_PREDICTIVO = Path(
    "resultados_sac/"
    "tercera_corrida_reactivo_mejorado/"
    "checkpoint_mejor_actor_reactivo_mejorado.pt"
)

DIRECTORIO_ENTRENAMIENTO_SAC_PREDICTIVO = Path(
    "resultados_sac/entrenamiento_sac_predictivo"
)

RUTA_MEJOR_ACTOR_SAC_PREDICTIVO = (
    DIRECTORIO_ENTRENAMIENTO_SAC_PREDICTIVO
    / "checkpoint_mejor_actor_sac_predictivo.pt"
)

RUTA_CHECKPOINT_FINAL_SAC_PREDICTIVO = (
    DIRECTORIO_ENTRENAMIENTO_SAC_PREDICTIVO
    / "checkpoint_final_completo_sac_predictivo.pt"
)

DIRECTORIO_GRAFICAS_ENTRENAMIENTO_SAC_PREDICTIVO = (
    DIRECTORIO_ENTRENAMIENTO_SAC_PREDICTIVO
    / "graficas_entrenamiento"
)

RUTA_CSV_VALIDACIONES_SAC_PREDICTIVO = (
    DIRECTORIO_ENTRENAMIENTO_SAC_PREDICTIVO
    / "validaciones_periodicas_50_semillas.csv"
)

RUTA_GRAFICA_VALIDACIONES_SAC_PREDICTIVO = (
    DIRECTORIO_ENTRENAMIENTO_SAC_PREDICTIVO
    / "validaciones_periodicas_50_semillas.png"
)

SEMILLA_DIAGNOSTICA_SAC_PREDICTIVO = 86000

RUTA_GRAFICA_DIAGNOSTICA_SAC_PREDICTIVO = (
    DIRECTORIO_ENTRENAMIENTO_SAC_PREDICTIVO
    / "trayectoria_diagnostica_mejor_actor_predictivo.png"
)

MOSTRAR_RESULTADOS_ENTRENAMIENTO_SAC_PREDICTIVO = True

EJECUTAR_VERIFICACION_PREVIA_SAC_PREDICTIVO = True

def entrenar_episodio_sac_predictivo(
    numero_episodio,
    semilla,
    actor,
    critico_q1,
    ...
    (
        entorno,
        observacion,
    ) = reiniciar_entorno_sac_predictivo(
        semilla=semilla
    )
    ...
    while not (
        ...
        (
            ...
            truncado,
            informacion_paso,
        ) = ejecutar_paso_entorno_sac_predictivo(
            entorno=entorno,
...
def entrenar_agente_sac_predictivo(
    numero_episodios,
    semilla_base,
    actor,
    critico_q1,
    ...
    for indice_episodio in range(
        ...
        resultado_episodio = (
            entrenar_episodio_sac_predictivo(
                numero_episodio=(
                    numero_episodio
                ),
...
def evaluar_episodio_sac_predictivo(
    semilla,
    actor,
    pasos_maximos=PASOS_MAXIMOS_SEGUIMIENTO,
    dt=DT,
...
def evaluar_actor_sac_predictivo_multisemilla(
    actor,
    semilla_base,
    numero_semillas,
    pasos_maximos=PASOS_MAXIMOS_SEGUIMIENTO,
    ...
    try:
        ...
        for indice in range(
            ...
            resultado_evaluacion = (
                evaluar_episodio_sac_predictivo(
                    semilla=semilla,
...
def guardar_checkpoint_mejor_actor_sac_predictivo(
    ruta_checkpoint,
    actor,
    episodio,
    resumen_validacion,
    ...
    if len(
        ...
        raise ValueError(
            "El criterio de selección del SAC predictivo "
            "mejorado debe contener siete elementos."
        )
    ...
    checkpoint = {
        ...
        "tipo": "mejor_actor_sac_predictivo",

        "variante_sac": "predictivo",

        "observacion_predictiva": True,
        ...
        "configuracion_predictiva": {
            "horizontes_parche": tuple(
                float(valor)
                for valor in HORIZONTES_PREDICCION_PARCHE_SAC
            ),
            "intensidades_parche": tuple(
                float(valor)
                for valor in INTENSIDADES_PREDICCION_PARCHE_SAC
            ),
            "horizonte_riesgo": float(
                HORIZONTE_RIESGO_DINAMICO_SAC
            ),
            "paso_prediccion": float(
                PASO_PREDICCION_DINAMICA_SAC
            ),
    ...
    if not ruta.is_file():
        ...
        raise RuntimeError(
            "No fue posible guardar el mejor actor SAC predictivo."
        )
...
def entrenar_agente_sac_predictivo_con_validacion_periodica(
    numero_episodios,
    episodios_entre_validaciones,
    semilla_entrenamiento,
    semilla_validacion,
    ...
    #
    # La recompensa moldeada no se utiliza para desempatar,
    # porque cambió con las mejoras del entorno predictivo.
    ...
    while episodios_completados < numero_episodios:
        ...
        resultado_bloque = entrenar_agente_sac_predictivo(
            numero_episodios=episodios_bloque,
        ...
        resultado_validacion = (
            evaluar_actor_sac_predictivo_multisemilla(
                actor=actor,
        ...
        if nuevo_mejor:
            ...
            informacion_mejor_checkpoint = (
                guardar_checkpoint_mejor_actor_sac_predictivo(
                    ruta_checkpoint=(
                        ruta_mejor_actor
                    ),
...
def agregar_metadatos_checkpoint_final_sac_predictivo(
    ruta_checkpoint,
    resultado_entrenamiento,
):
    ...
    checkpoint[
        "tipo"
    ] = "sac_predictivo_entrenamiento_completo"
    ...
    checkpoint[
        ...
        "episodios": int(
            NUMERO_EPISODIOS_ENTRENAMIENTO_SAC_PREDICTIVO
        ),

        "semilla_entrenamiento": int(
            SEMILLA_BASE_ENTRENAMIENTO_SAC_PREDICTIVO
        ),

        "semilla_validacion": int(
            SEMILLA_BASE_VALIDACION_SAC_PREDICTIVO
        ),

        "numero_semillas_validacion": int(
            NUMERO_SEMILLAS_VALIDACION_SAC_PREDICTIVO
        ),

        "episodios_entre_validaciones": int(
            EPISODIOS_ENTRE_VALIDACIONES_SAC_PREDICTIVO
        ),

        "tasa_actor": float(
            TASA_APRENDIZAJE_ACTOR_SAC_PREDICTIVO
        ),

        "tasa_criticos": float(
            TASA_APRENDIZAJE_CRITICOS_SAC_PREDICTIVO
        ),

        "tasa_alpha": float(
            TASA_APRENDIZAJE_ALPHA_SAC_PREDICTIVO
        ),
    }
...
# ==========================================================
# VERIFICACIÓN PREVIA DE LA RAMA PREDICTIVA
# ==========================================================
def verificar_entrenamiento_sac_predictivo():
    ...
    print(
        "VERIFICACIÓN PREVIA DEL SAC PREDICTIVO"
    )
    ...
    valores_reactivos = np.unique(
        observacion_reactiva[
            "parche"
        ][1]
    )

    valores_predictivos = np.unique(
        observacion_predictiva[
            "parche"
        ][1]
    )

    canal_reactivo_binario = np.all(
        np.isin(
            valores_reactivos,
            [
                0.0,
    ...
    intensidades_permitidas = np.asarray(
        (
            0.0,
        )
        + INTENSIDADES_PREDICCION_PARCHE_SAC,
    ...
    canal_predictivo_valido = np.all(
        np.isin(
            valores_predictivos,
            intensidades_permitidas,
        )
    )

    accion_prueba = np.asarray(
        [
            0.0,
            0.0,
        ],
    ...
    claves_predictivas = {
        ...
        "recompensa_predictiva",
        "clearance_dinamico_predicho_minimo",
        "ttc_dinamico_predicho",
        "velocidad_cierre_dinamica_maxima",
        "colision_dinamica_predicha",
        "penalizacion_riesgo_dinamico_predictivo",
    }

    paso_predictivo_integrado = (
        claves_predictivas.issubset(
            informacion.keys()
        )
        and informacion[
            "modo_sac"
    ...
    registro_predictivo_actualizado = (
        len(
            entorno_predictivo[
                "registro"
            ][
    ...
    todo_correcto = (
        mismo_escenario
        and formas_correctas
        and canal_estatico_igual
        and canal_reactivo_binario
        and canal_predictivo_valido
        and paso_predictivo_integrado
        and registro_predictivo_actualizado
    )

    print(
        "Mismo escenario reactivo/predictivo:",
        mismo_escenario,
    )
    ...
    print(
        "Canal estático idéntico:",
        canal_estatico_igual,
    )

    print(
        "Canal reactivo binario:",
        canal_reactivo_binario,
    )

    print(
        "Canal predictivo válido:",
        canal_predictivo_valido,
    )

    print(
        "Paso predictivo integrado:",
        paso_predictivo_integrado,
    )

    print(
        "Registro predictivo actualizado:",
        registro_predictivo_actualizado,
    )
    ...
    if not todo_correcto:
        ...
        raise RuntimeError(
            "La verificación previa del SAC predictivo "
            "no fue satisfactoria."
        )

    print(
        "RESULTADO DE VERIFICACIÓN: TODO CORRECTO"
    )
...
# ==========================================================
# PRUEBA INDEPENDIENTE DEL SAC PREDICTIVO
# ==========================================================

NUMERO_SEMILLAS_PRUEBA_INDEPENDIENTE_SAC_PREDICTIVO = 100

SEMILLA_BASE_PRUEBA_INDEPENDIENTE_SAC_PREDICTIVO = 87000

FRECUENCIA_IMPRESION_PRUEBA_INDEPENDIENTE_SAC_PREDICTIVO = 10

RUTA_ACTOR_PRUEBA_INDEPENDIENTE_SAC_PREDICTIVO = Path(
    "resultados_sac/entrenamiento_sac_predictivo/"
    "checkpoint_mejor_actor_sac_predictivo.pt"
)

DIRECTORIO_PRUEBA_INDEPENDIENTE_SAC_PREDICTIVO = Path(
    "resultados_sac/prueba_independiente_sac_predictivo"
)

RUTA_CSV_PRUEBA_INDEPENDIENTE_SAC_PREDICTIVO = (
    DIRECTORIO_PRUEBA_INDEPENDIENTE_SAC_PREDICTIVO
    / "resultados_100_semillas.csv"
)

RUTA_CSV_FALLOS_PRUEBA_INDEPENDIENTE_SAC_PREDICTIVO = (
    DIRECTORIO_PRUEBA_INDEPENDIENTE_SAC_PREDICTIVO
    / "semillas_fallidas.csv"
)

RUTA_RESUMEN_PRUEBA_INDEPENDIENTE_SAC_PREDICTIVO = (
    DIRECTORIO_PRUEBA_INDEPENDIENTE_SAC_PREDICTIVO
    / "resumen_prueba_independiente.txt"
)

RUTA_GRAFICA_PRUEBA_INDEPENDIENTE_SAC_PREDICTIVO = (
    DIRECTORIO_PRUEBA_INDEPENDIENTE_SAC_PREDICTIVO
    / "distribucion_resultados_100_semillas.png"
)

TASA_EXITO_MINIMA_PRUEBA_SAC_PREDICTIVO = 0.85
TASA_COLISION_DINAMICA_MAXIMA_PRUEBA_SAC_PREDICTIVO = 0.10
TASA_COLISION_ESTATICA_MAXIMA_PRUEBA_SAC_PREDICTIVO = 0.10
TASA_FUERA_MAPA_MAXIMA_PRUEBA_SAC_PREDICTIVO = 0.05
TASA_TIMEOUT_MAXIMA_PRUEBA_SAC_PREDICTIVO = 0.05

MOSTRAR_GRAFICA_PRUEBA_INDEPENDIENTE_SAC_PREDICTIVO = True


def guardar_csv_diccionarios_sac_predictivo(
    ruta,
    registros,
):
    ...
    if len(registros) == 0:
        raise ValueError("No hay registros para guardar en CSV.")
...
def crear_grafica_prueba_independiente_sac_predictivo(
    resumen,
    ruta_guardado,
    mostrar=True,
):
    ...
    valores = [
        conteos["meta"],
        conteos["colision_estatica"],
        conteos["colision_dinamica"],
        conteos["fuera_mapa"],
        conteos["timeout"],
    ]
    ...
    eje.set_title(
        "Prueba independiente del SAC predictivo\n"
        f"Éxito: {100.0 * resumen['tasa_exito']:.2f}%"
    )
    eje.set_ylabel("Número de episodios")
    eje.set_ylim(0, max(valores) + 10)
    ...
    figura.savefig(ruta_guardado, dpi=300, bbox_inches="tight")
...
def construir_texto_resumen_prueba_independiente_sac_predictivo(
    resumen,
    criterios,
    semillas_fallidas,
    episodio_actor,
    ...
    lineas = [
        ...
        "=" * 72,
        "",
        f"Checkpoint: {RUTA_ACTOR_PRUEBA_INDEPENDIENTE_SAC_PREDICTIVO}",
        f"Episodio del actor: {episodio_actor}",
        "Política determinista: True",
        "Entrenamiento durante la prueba: False",
        "Observación predictiva: True",
        "TTC: True",
        "",
        f"Semillas: {resumen['semilla_base']} a "
        f"{resumen['semilla_base'] + resumen['numero_semillas'] - 1}",
        f"Número de semillas: {resumen['numero_semillas']}",
        "Semillas finales 70000–70099 reservadas: True",
        "",
        f"Éxitos: {resumen['numero_exitos']}/{resumen['numero_semillas']}",
        ...
        f"Error medio de ruta promedio: {resumen['error_medio_ruta_promedio']} m",
        ...
        "",
        "EPISODIOS EXITOSOS",
        f"Tiempo promedio: {resumen['tiempo_promedio_exitos']} s",
        f"Longitud promedio: {resumen['longitud_promedio_exitos']} m",
        f"Error de ruta promedio: {resumen['error_ruta_promedio_exitos']} m",
        f"Clearance mínimo promedio: {resumen['clearance_minimo_promedio_exitos']} m",
        "",
        "CRITERIOS",
        f"Éxito >= 85%: {criterios['exito']}",
        f"Colisión dinámica <= 10%: {criterios['colision_dinamica']}",
        f"Colisión estática <= 10%: {criterios['colision_estatica']}",
        f"Fuera del mapa <= 5%: {criterios['fuera_mapa']}",
        f"Timeout <= 5%: {criterios['timeout']}",
        f"TODOS LOS CRITERIOS: {criterios['todos']}",
        "",
        "SEMILLAS FALLIDAS",
        str(semillas_fallidas),
    ]
...
# ==========================================================
# FUNCIÓN PRINCIPAL: PRUEBA INDEPENDIENTE SAC PREDICTIVO
# ==========================================================
def main():

    print("\n" + "=" * 80)
    print("PRUEBA INDEPENDIENTE DEL SAC PREDICTIVO")
    print("=" * 80)

    plt.close("all")

    # ------------------------------------------------------
    # 1. Verificar que el checkpoint existe
    # ------------------------------------------------------

    ruta_checkpoint = (
        RUTA_ACTOR_PRUEBA_INDEPENDIENTE_SAC_PREDICTIVO
    )

    if not ruta_checkpoint.is_file():
        raise FileNotFoundError(
            "No se encontró el mejor actor predictivo:\n"
            f"{ruta_checkpoint}"
        )
    ...
    if not campos_requeridos.issubset(checkpoint.keys()):
        raise KeyError(
            "El checkpoint predictivo no contiene los campos "
            "mínimos requeridos."
        )

    checkpoint_correcto = (
        checkpoint.get("tipo") == "mejor_actor_sac_predictivo"
        and checkpoint.get("variante_sac") == "predictivo"
        and bool(checkpoint.get("observacion_predictiva", False))
        and bool(checkpoint.get("usa_ttc", False))
    )

    if not checkpoint_correcto:
        raise ValueError(
            "El archivo encontrado no corresponde al mejor "
            "actor SAC predictivo."
        )

    episodio_actor = int(checkpoint["episodio"])

    # ------------------------------------------------------
    # 2. Crear y cargar el actor estrictamente
    # ------------------------------------------------------

    actor = crear_actor_sac(dispositivo=DISPOSITIVO_SAC)
    ...
    carga_estricta_correcta = (
        len(resultado_carga.missing_keys) == 0
        and len(resultado_carga.unexpected_keys) == 0
    )

    if not carga_estricta_correcta:
        raise RuntimeError(
            "El actor predictivo no se cargó estrictamente."
        )
    ...
    parametros_finitos = all(
        torch.isfinite(parametro).all().item()
        for parametro in actor.parameters()
    )
    ...
    print("Dispositivo:", DISPOSITIVO_SAC)
    print(
        "Número de parámetros:",
        sum(parametro.numel() for parametro in actor.parameters()),
    )
    print("Política determinista: True")
    print("Entrenamiento durante la prueba: False")
    print("Predicción: True")
    print("TTC: True")

    semilla_base = (
        SEMILLA_BASE_PRUEBA_INDEPENDIENTE_SAC_PREDICTIVO
    )
    numero_semillas = (
        NUMERO_SEMILLAS_PRUEBA_INDEPENDIENTE_SAC_PREDICTIVO
    )
    ...
    print(
        ...
        semilla_base,
        "a",
        semilla_base + numero_semillas - 1,
    )
    print(
        "Las semillas finales 70000–70099 permanecen "
        "reservadas."
    )

    # ------------------------------------------------------
    # 4. Ejecutar evaluación independiente
    # ------------------------------------------------------
    ...
    resultado_evaluacion = (
        evaluar_actor_sac_predictivo_multisemilla(
            ...
            dispositivo=DISPOSITIVO_SAC,
            frecuencia_impresion=(
                FRECUENCIA_IMPRESION_PRUEBA_INDEPENDIENTE_SAC_PREDICTIVO
            ),
        )
    )

    resultados_individuales = resultado_evaluacion[
        "resultados_individuales"
    ]
    resumen = resultado_evaluacion["resumen"]
    ...
    criterios = {
        "exito": (
            resumen["tasa_exito"]
            >= TASA_EXITO_MINIMA_PRUEBA_SAC_PREDICTIVO
        ),
        "colision_dinamica": (
            resumen["tasa_colision_dinamica"]
            <= TASA_COLISION_DINAMICA_MAXIMA_PRUEBA_SAC_PREDICTIVO
        ),
        "colision_estatica": (
            resumen["tasa_colision_estatica"]
            <= TASA_COLISION_ESTATICA_MAXIMA_PRUEBA_SAC_PREDICTIVO
        ),
        "fuera_mapa": (
            resumen["tasa_fuera_mapa"]
            <= TASA_FUERA_MAPA_MAXIMA_PRUEBA_SAC_PREDICTIVO
        ),
        "timeout": (
            resumen["tasa_timeout"]
            <= TASA_TIMEOUT_MAXIMA_PRUEBA_SAC_PREDICTIVO
        ),
    }
    ...
    semillas_fallidas = [
        int(registro["semilla"])
        for registro in resultados_individuales
        if int(registro["exito"]) == 0
    ]
    ...
    DIRECTORIO_PRUEBA_INDEPENDIENTE_SAC_PREDICTIVO.mkdir(
        parents=True,
        exist_ok=True,
    )

    guardar_csv_diccionarios_sac_predictivo(
        RUTA_CSV_PRUEBA_INDEPENDIENTE_SAC_PREDICTIVO,
        resultados_individuales,
    )

    if len(registros_fallidos) > 0:
        guardar_csv_diccionarios_sac_predictivo(
            RUTA_CSV_FALLOS_PRUEBA_INDEPENDIENTE_SAC_PREDICTIVO,
            registros_fallidos,
        )
    else:
        with RUTA_CSV_FALLOS_PRUEBA_INDEPENDIENTE_SAC_PREDICTIVO.open(
            "w",
            encoding="utf-8",
        ) as archivo:
            archivo.write("semilla,resultado\n")

    texto_resumen = (
        construir_texto_resumen_prueba_independiente_sac_predictivo(
            resumen=resumen,
            criterios=criterios,
            semillas_fallidas=semillas_fallidas,
            episodio_actor=episodio_actor,
        )
    )

    RUTA_RESUMEN_PRUEBA_INDEPENDIENTE_SAC_PREDICTIVO.write_text(
        texto_resumen,
        encoding="utf-8",
    )

    crear_grafica_prueba_independiente_sac_predictivo(
        resumen=resumen,
        ruta_guardado=(
            RUTA_GRAFICA_PRUEBA_INDEPENDIENTE_SAC_PREDICTIVO
        ),
        mostrar=(
            MOSTRAR_GRAFICA_PRUEBA_INDEPENDIENTE_SAC_PREDICTIVO
        ),
    )
    ...
    actor_sin_gradientes = all(
        parametro.grad is None
        for parametro in actor.parameters()
    )

    numero_resultados_correcto = (
        len(resultados_individuales) == numero_semillas
    )

    conteos_completos = (
        sum(resumen["conteos_resultados"].values())
        == numero_semillas
    )

    semillas_correctas = (
        resultados_individuales[0]["semilla"] == semilla_base
        and resultados_individuales[-1]["semilla"]
        == semilla_base + numero_semillas - 1
    )
    ...
    print("\n" + "=" * 80)
    print("RESULTADO DE LA PRUEBA INDEPENDIENTE PREDICTIVA")
    print("=" * 80)

    print("\nRESULTADOS GENERALES")
    print(
        "Éxitos:",
        f"{resumen['numero_exitos']}/{resumen['numero_semillas']}",
    )
    print(
        "Tasa de éxito:",
        f"{100.0 * resumen['tasa_exito']:.2f}%",
    )
    print(
        "IC Wilson 95%:",
        f"[{100.0 * resumen['intervalo_exito_95_inferior']:.2f}%, "
    ...
    print(
        "Colisión estática:",
        f"{100.0 * resumen['tasa_colision_estatica']:.2f}%",
    )
    print(
        "Colisión dinámica:",
        f"{100.0 * resumen['tasa_colision_dinamica']:.2f}%",
    )
    print(
        "Fuera del mapa:",
        f"{100.0 * resumen['tasa_fuera_mapa']:.2f}%",
    )
    print(
        "Timeout:",
        f"{100.0 * resumen['tasa_timeout']:.2f}%",
    )
    ...
    print(
        ...
        resumen["distancia_final_media"],
        "m",
    )
    print(
        "Error medio de ruta promedio:",
        resumen["error_medio_ruta_promedio"],
        "m",
    )
    print(
        "Clearance mínimo promedio:",
        resumen["clearance_minimo_promedio"],
        "m",
    )
    print(
        "Clearance mínimo absoluto:",
        resumen["clearance_minimo_absoluto"],
        "m",
    )
    ...
    print(
        "Tiempo promedio:",
        resumen["tiempo_promedio_exitos"],
        "s",
    )
    print(
        "Longitud promedio:",
        resumen["longitud_promedio_exitos"],
        "m",
    )
    print(
        "Error de ruta promedio:",
        resumen["error_ruta_promedio_exitos"],
        "m",
    )
    print(
        "Clearance mínimo promedio:",
        resumen["clearance_minimo_promedio_exitos"],
        "m",
    )
    ...
    print(
        "Colisión dinámica <= 10%:",
        criterios["colision_dinamica"],
    )
    print(
        "Colisión estática <= 10%:",
        criterios["colision_estatica"],
    )
    ...
    print("\nARCHIVOS")
    print("CSV completo:", RUTA_CSV_PRUEBA_INDEPENDIENTE_SAC_PREDICTIVO)
    print("CSV de fallos:", RUTA_CSV_FALLOS_PRUEBA_INDEPENDIENTE_SAC_PREDICTIVO)
    print("Resumen:", RUTA_RESUMEN_PRUEBA_INDEPENDIENTE_SAC_PREDICTIVO)
    print("Gráfica:", RUTA_GRAFICA_PRUEBA_INDEPENDIENTE_SAC_PREDICTIVO)

    print("\nVERIFICACIONES TÉCNICAS")
    print("Checkpoint correcto:", checkpoint_correcto)
    print("Carga estricta correcta:", carga_estricta_correcta)
    ...
    if tecnicamente_correcto:
        print("\nRESULTADO TÉCNICO: PRUEBA INDEPENDIENTE COMPLETADA")
    else:
        print("\nRESULTADO TÉCNICO: HAY ALGO POR CORREGIR")

    if criterios["todos"]:
        print("RESULTADO CIENTÍFICO: SAC PREDICTIVO APTO")
    else:
        print("RESULTADO CIENTÍFICO: SAC PREDICTIVO AÚN NO APTO")
